...
export function NoteCanvas({ note, onSave }: NoteCanvasProps) {
  ...
  const getPathPoints = (path: (string | number)[][]) => {
    const points: { x: number; y: number }[] = [];
    let current = { x: 0, y: 0 };
    let start = { x: 0, y: 0 };
    ...
    path.forEach(segment => {
      ...
      switch (command) {
        case "M": {
          current = resolvePoint(values[0] as number, values[1] as number, relative);
          start = { ...current };
          points.push({ ...current });
        ...
        case "H": {
          current = {
            x: relative ? current.x + (values[0] as number) : (values[0] as number),
            y: current.y,
          };
          points.push({ ...current });
          previousControl = null;
        ...
        case "V": {
          current = {
            x: current.x,
            y: relative ? current.y + (values[0] as number) : (values[0] as number),
          };
          points.push({ ...current });
          previousControl = null;
          break;
        ...
        case "C": {
          const control1 = resolvePoint(values[0] as number, values[1] as number, relative);
          const control2 = resolvePoint(values[2] as number, values[3] as number, relative);
          const end = resolvePoint(values[4] as number, values[5] as number, relative);
          points.push(...sampleCubicPoints(current, control1, control2, end));
          current = end;
        ...
        case "S": {
          const control1 = previousCommand === "C" || previousCommand === "S"
            ...
            : { ...current };
          const control2 = resolvePoint(values[0] as number, values[1] as number, relative);
          const end = resolvePoint(values[2] as number, values[3] as number, relative);
          points.push(...sampleCubicPoints(current, control1, control2, end));
          current = end;
          previousControl = control2;
          break;
        ...
        case "Q": {
          const control = resolvePoint(values[0] as number, values[1] as number, relative);
          const end = resolvePoint(values[2] as number, values[3] as number, relative);
          points.push(...sampleQuadraticPoints(current, control, end));
          current = end;
        ...
        case "T": {
          const control = previousCommand === "Q" || previousCommand === "T"
            ? { x: current.x * 2 - (previousControl?.x ?? current.x), y: current.y * 2 - (previousControl?.y ?? current.y) }
            : { ...current };
          const end = resolvePoint(values[0] as number, values[1] as number, relative);
          points.push(...sampleQuadraticPoints(current, control, end));
          current = end;
          previousControl = control;
          break;
  ...
  const segmentHitsObject = (obj: FabricObject, start: { x: number; y: number }, end: { x: number; y: number }, radius: number) => {
    ...
    if (obj.type === "path") {
      ...
      const pathPoints = transformObjectPoints(pathObj, getPathPoints(pathObj.path));
  ...
  }, [settings.strokeWidth, settings.eraserType, lineIntersectsRect, segmentHitsObject]);
