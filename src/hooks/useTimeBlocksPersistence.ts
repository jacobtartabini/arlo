...
export function useTimeBlocksPersistence() {
  const fetchTimeBlocksForDate = async (date: Date): Promise<TimeBlock[]> => {
    ...
    // For now, fetch all and filter client-side (can be optimized with a dedicated edge function)
  ...
  const fetchTimeBlocksForTask = async (taskId: string): Promise<TimeBlock[]> => {
    if (!isAuthenticated()) return [];
  ...
  const updateTimeBlock = async (
    ...
    if (updates.actualDurationMinutes !== undefined) dbUpdates.actual_duration_minutes = updates.actualDurationMinutes;
