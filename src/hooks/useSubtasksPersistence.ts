...
export function useSubtasksPersistence() {
  const fetchSubtasksForTask = async (taskId: string): Promise<Subtask[]> => {
  ...
  const fetchSubtasksForTasks = async (taskIds: string[]): Promise<Map<string, Subtask[]>> => {
  ...
  const toggleSubtask = async (id: string, done: boolean): Promise<boolean> => {
    return updateSubtask(id, { done });
  };

  const reorderSubtasks = async (
    subtasks: { id: string; orderIndex: number }[]
  ): Promise<boolean> => {
    if (!isAuthenticated()) return false;
