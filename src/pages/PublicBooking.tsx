...
const PublicBookingPage = () => {
  ...
  const timeSlots = React.useMemo(() => {
    ...
    return slots.map(formatSlotTimeRange);
  ...
  const handleConfirm = (value: { date?: Date; time?: string }) => {
    ...
    const slots = availableSlotsByDate.get(key) ?? [];
    const match = slots.find(slot => formatSlotTimeRange(slot) === value.time);
    if (!match) return;
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-background px-4 py-12">
      ...
      {lastBookedSlot ? (
        <div className="mt-6 text-sm text-muted-foreground">
          Meeting confirmed for {formatSlotLabel(lastBookedSlot)}.
        </div>
      ) : null}
    </div>
  );
