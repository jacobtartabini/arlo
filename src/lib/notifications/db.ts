...
function dbToPreferences(db: Record<string, unknown>): NotificationPreferences {
  return {
    ...
    typeToggles: (db.type_toggles as Record<NotificationType, boolean>) ?? { system: true, chat: true, calendar: true, security: true, habits: true, tasks: true },
...
export async function fetchPreferences(): Promise<NotificationPreferences | null> {
  ...
  if (error) {
    ...
    return {
      ...
      toastEnabled: true,
      pushEnabled: false,
      typeToggles: { system: true, chat: true, calendar: true, security: true, habits: true, tasks: true },
